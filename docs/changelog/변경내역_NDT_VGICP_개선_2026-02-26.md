...
최종 개선 후, 각 Factor별 성능 지표 변화는 다음과 같습니다.
...
| **NDT** | **92** | **0.095** | **0.725** | **정확도 대폭 향상** |
| LOAM | 8 | 0.225 | 0.775 | 유지 |

---

## 9. 변경 파일 세부 내용
1. `integrated_ndt_factor_impl.hpp`: NDT 수식 리팩토링 및 Magnusson 표준 모델 적용.
...
5. `src/main.cpp`: Voxel 해상도 조정(1.0m), `lambdaLowerBound` 설정, LOAM 불필요 구간 스킵.

---

## 10. 향후 개선 가능 사항
- **VGICP**: 입력 포인트 클라우드의 다운샘플링(0.25m 수준)을 적용하여 연산 속도를 높이고, `max_correspondence_distance` 제약을 추가할 예정입니다.
- **NDT**: 현재 0.1인 `outlier_ratio`를 0.55 정도로 튜닝하고, 논문의 식 6.13에 명시된 H2 항(Hessian 근사 보완) 추가를 고려 중입니다. 또한 More-Thuente 방식의 Line search 도입도 검토 대상입니다.
